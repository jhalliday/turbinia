 ...
 * @since 2019-04
 */
public class AppendOnlyLog implements Iterable<ByteBuffer> {
     ...
     * Use canAccept(n) to determine if there is sufficient space remaining.
     * <p>
     ...
     * @throws BufferOverflowException If there is insufficient space in this log
     * @see {@link #tryPut(ByteBuffer)}
     */
    public void put(ByteBuffer src) {
        logger.entry(src);

        boolean success = tryPut(src);
        if (!success) {
     ...
     * Prefer canAccept(n) to accurately determine if a write of known size will fit.
     *
